...
parse_tickers <- utils_parse_tickers
.http_get <- utils_http_get
.read_finviz_table <- utils_read_finviz_table
.num <- utils_as_num
.as_pct <- utils_as_pct

# ------------------- Tickers por defecto (módulo estándar) -------------------
DEFAULT_TICKERS <- c(
  "NVDA", "MSFT", "AAPL", "AMZN", "META", "AVGO", "GOOGL", "TSLA",
  "JPM", "ORCL", "WMT", "LLY", "V", "MA", "NFLX", "XOM", "JNJ",
  "HD", "COST", "ABBV", "PLTR", "BAC", "PG", "CVX", "UNH", "AMD",
  "CRM", "IBM", "GS", "BABA", "TSM", "ASML", "AXP", "MCD", "DIS",
  "MRK", "UBER", "PYPL", "INTC", "COIN"
)

default_tickers_text <- function() {
  paste(DEFAULT_TICKERS, collapse = ", ")
}
...
# Busca claves originales de Finviz ("P/E", "Gross Margin") directamente en el raw_list
# tal como lo hace el modulo institucional. Incluye fallback para caché sanitizado.
getv_robust <- function(raw_list, patterns) {
  if (is.null(raw_list) || length(raw_list) == 0) {
    return(NA_character_)
  }
  # 1. Buscar clave exacta (modo institucional - claves originales)
  for (k in patterns) {
    v <- raw_list[[k]]
  ...
  # 2. Fallback: buscar con variacion make.names (cache legado sanitizado)
  for (k in patterns) {
    k_san <- make.names(gsub("[$/%().-]", ".", k))
    v <- raw_list[[k_san]]
    if (!is.null(v) && length(v) > 0 && !is.na(v) && nzchar(as.character(v))) {
      return(as.character(v))
    }
...
build_indicator_table <- function(tk, m, raw) {
  ...
  vals <- list(
    # valoracion
    "P/E"         = .num(getv_robust(r, c("P/E",         "P.E",        "P.E."))),
    "PEG"         = .num(getv_robust(r, c("PEG"))),
    "Forward P/E" = .num(getv_robust(r, c("Forward P/E", "Forward.P.E", "Forward.P.E."))),
    ...
    # crecimiento
    "EPS Y/Y TTM"  = .num(getv_robust(r, c("EPS this Y",   "EPS (ttm)", "EPS.this.Y", "EPS..this.Y", "EPS.TTM"))),
    "Sales Y/Y TTM"= .num(getv_robust(r, c("Sales past 5Y","Sales.past.5Y", "Sales this Y", "Sales.this.Y"))),
    "EPS next Y"   = .num(getv_robust(r, c("EPS next Y",   "EPS.next.Y"))),
    "EPS next 5Y"  = .num(getv_robust(r, c("EPS next 5Y",  "EPS.next.5Y"))),
    "Sales Q/Q"    = .num(getv_robust(r, c("Sales Q/Q",    "Sales.Q.Q"))),
    "EPS Q/Q"      = .num(getv_robust(r, c("EPS Q/Q",      "EPS.Q.Q"))),

    # rentabilidad
    "Oper. Margin" = .num(getv_robust(r, c("Oper. Margin", "Oper..Margin"))),
    "Profit Margin"= .num(getv_robust(r, c("Profit Margin","Profit.Margin"))),
    "ROE"          = .num(getv_robust(r, c("ROE"))),
    "ROIC"         = .num(getv_robust(r, c("ROI", "ROIC", "ROI."))),
    "Gross Margin" = .num(getv_robust(r, c("Gross Margin", "Gross.Margin"))),
    "ROA"          = .num(getv_robust(r, c("ROA"))),

    # salud
    "Debt/Eq"      = .num(getv_robust(r, c("Debt/Eq", "Debt.Eq"))),
    "Current Ratio"= .num(getv_robust(r, c("Current Ratio", "Current.Ratio"))),

    # tecnica (calculados de Yahoo, no de Finviz)
    ...
    # sentimiento
    "Short Float"  = .num(getv_robust(r, c("Short Float",  "Short.Float"))),
    "Inst Own"     = .num(getv_robust(r, c("Inst Own",     "Inst.Own"))),
    "Insider Trans"= .num(getv_robust(r, c("Insider Trans","Insider.Trans"))),
    ...
    "Upside to Target %" = if (!is.na(price)) {
      t <- .num(getv_robust(r, c("Target Price", "Target.Price")))
...
analyze_stock <- function(tk, m, invest_prof = "conservador", macro_ctx = "normal") {
  r <- m$raw %||% list()
  ...
  # IMPROVEMENT 4: MACRO CONTEXT WEIGHTS
  indicators <- indicators %>%
    mutate(PesoAdj = case_when(
      macro_ctx == "defensivo" & `Categoría` %in% c("salud", "rentabilidad") ~ Peso * 1.5,
      macro_ctx == "defensivo" & `Categoría` == "crecimiento" ~ Peso * 0.75,
      macro_ctx == "expansivo" & `Categoría` == "crecimiento" ~ Peso * 1.5,
      macro_ctx == "expansivo" & `Categoría` %in% c("salud", "rentabilidad") ~ Peso * 0.75,
      TRUE ~ Peso
    ))
  ...
  cat_scores <- indicators %>%
    ...
    group_by(`Categoría`) %>%
    summarise(
      score = sum(Score_Num * PesoAdj) / sum(PesoAdj),
      .groups = "drop"
    )
  ...
  if (nrow(valid_inds) > 0) {
    total <- round(sum(valid_inds$Score_Num * valid_inds$PesoAdj) / sum(valid_inds$PesoAdj))
  } else {
    total <- 50
  }
  ...
  # IMPROVEMENT 1: SECTOR BASED DEBT/EQ LIMIT
  sector <- getv_robust(r, c("Sector"))
  is_financial_or_utility <- isTRUE(grepl("Financial|Util", sector, ignore.case = TRUE))
  debt_val <- .num(getv_robust(r, c("Debt/Eq", "Debt.Eq")))

  debt_flag <- is_red("Debt/Eq")
  if (is_financial_or_utility && !is.na(debt_val)) {
    if (debt_val <= 3.0) debt_flag <- FALSE
  }

  # IMPROVEMENT 3: VALUE INVESTOR PROFILE (Ignore SMA200 if val & prof excellent)
  sma200_flag <- is_red("SMA200")
  if (invest_prof == "value" && sc_val >= 75 && sc_prof >= 75) {
    sma200_flag <- FALSE
  }

  danger_flag <- debt_flag || is_red("ROIC") || sma200_flag
  ...
  # IMPROVEMENT 2: LIQUIDITY / MARKET CAP WARNING
  mcap_str <- getv_robust(r, c("Market Cap", "Market.Cap"))
  is_microcap <- FALSE
  if (!is.na(mcap_str)) {
    mcap_num <- suppressWarnings(as.numeric(gsub("[A-Za-z,\\s$]", "", mcap_str)))
    if (!is.na(mcap_num)) {
      if (grepl("B", mcap_str)) mcap_num <- mcap_num * 1000
      if (grepl("M", mcap_str) && mcap_num < 300) is_microcap <- TRUE
      if (!grepl("[BM]", mcap_str) && mcap_num < 300000000) is_microcap <- TRUE
    }
  }

  if (is_microcap && grepl("Comprar", verdict)) {
    verdict <- paste0(verdict, " ⚠️(Micro-Cap)")
  }
  ...
  Target <- getv_robust(r, c("Target Price", "Target.Price"))
  tgt <- .num(Target)
...
run_pipeline <- function(tickers, progress_cb = NULL, investor_profile = "conservador", macro_context = "normal") {
  n <- length(tickers)
  ...
  if (length(items_to_parallel) > 0) {
    ...
    # Siempre usar parallel para evitar bloquear la sesión de Shiny
    use_parallel <- n_p >= 1
    if (use_parallel) {
      chunk_size <- 5
      ...
      chunks <- list(names(items_to_parallel))
    }
    ...
    for (i in seq_along(chunks)) {
      ...
      if (use_parallel) {
        batch_res <- furrr::future_map(batch_keys, function(k) {
          item <- items_to_parallel[[k]]
          if (item$type == "full") return(worker_fetch_full(item$ticker))
          return(worker_fetch_yahoo_only(item$ticker))
        }, .options = furrr::furrr_options(seed = TRUE))
      } else {
        batch_res <- purrr::map(batch_keys, function(k) {
          item <- items_to_parallel[[k]]
          if (item$type == "full") return(worker_fetch_full(item$ticker))
          return(worker_fetch_yahoo_only(item$ticker))
        })
    ...
    tryCatch(
      {
        ...
        r <- try(analyze_stock(tk, m, invest_prof = investor_profile, macro_ctx = macro_context), silent = TRUE)

        if (inherits(r, "try-error") || is.null(r)) {
          # Loguear el error real para debugging en servidor
          err_msg <- if (inherits(r, "try-error")) conditionMessage(attr(r, "condition")) else "resultado NULL"
          message(sprintf("[Pipeline] analyze_stock FALLO en '%s': %s", tk, err_msg))
          stop(paste("Analysis failed:", err_msg))
        } else {
      ...
      error = function(e) {
        # Loguear y guardar fila neutral con el ERROR REAL visible para debugging
        message(sprintf("[Pipeline] ERROR FINAL en ticker '%s': %s", tk, e$message))
        neutral <- tibble::tibble(
          Fecha = as.Date(Sys.Date()), Ticker = tk, Rank = NA_integer_,
          `Puntaje (0-100)` = 50, Veredicto = "Sin Datos",
          `Conclusión` = paste0("ERR: ", substring(e$message, 1, 120)),
...
run_pipeline_default <- function(tickers = NULL, progress_cb = NULL, investor_profile = "conservador", macro_context = "normal") {
  tks <- tickers
  ...
  run_pipeline(tks, progress_cb = progress_cb, investor_profile = investor_profile, macro_context = macro_context)
}
