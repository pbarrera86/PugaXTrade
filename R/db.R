...
db_register_user <- function(username, email, name, country, phone, referred_by = NULL, password = NULL) {
  .with_conn({
    ...
    tryCatch({
      DBI::dbBegin(con)
    }, error = function(e) message("Warning: dbBegin failed: ", e$message))

    # Defensive Rollback on Exit
    on.exit(try(DBI::dbRollback(con), silent = TRUE), add = TRUE)

    # 1. Referred By Setup
    safe_refferal_id <- NA_character_
    if (!is.null(referred_by) && nzchar(as.character(referred_by))) {
        safe_refferal_id <- as.character(referred_by)
    } else {
      # Default Referrer (Admin)
      admin_u <- DBI::dbGetQuery(con, "select id from users where lower(username)='pedrobp86' limit 1")
      if (nrow(admin_u) > 0) safe_refferal_id <- as.character(admin_u$id[[1]])
    }

    # 2. Sequential Referral Code
    ref_code_val <- NULL
    tryCatch({
      for (k in 1:3) {
        cand <- db_generate_next_ref_code(con)
        q_check <- DBI::dbGetQuery(con, "select 1 from users where referral_code=$1 limit 1", params = list(as.character(cand)))
        if (nrow(q_check) == 0) {
          ref_code_val <- as.character(cand)
          break
        }
        Sys.sleep(0.05)
      }
    }, error = function(e) message("Ref code error: ", e$message))
    
    if (is.null(ref_code_val)) ref_code_val <- paste0("ref", as.character(as.integer(Sys.time())))

    # 3. Hash
    final_h <- if (!is.null(password) && nzchar(password)) sodium::password_store(password) else sodium::password_store(rnd_token(16))
    ...
    tryCatch({
      u <- DBI::dbGetQuery(con,
        ...
        params = list(
          ...
          as.character(ifelse(is.na(phone), "", phone)),
          as.character(final_h),
          as.character(Sys.Date() + 7), # Standardize date to char for DB
          as.character(ref_code_val),
          as.character(safe_refferal_id),
          as.character(rnd_token(32))
        )
      )
      
      DBI::dbCommit(con)
      return(list(ok = TRUE, user = u))
    }, error = function(e) {
      try(DBI::dbRollback(con), silent = TRUE)
      message("Fatal DB Registration Error: ", e$message)
      return(list(ok = FALSE, message = paste("Error DB:", e$message)))
    })
  })
}
...
db_ensure_referral_code <- function(user_id) {
  .with_conn({
    ...
    if (nrow(u) > 0 && (is.null(u$referral_code[[1]]) || is.na(u$referral_code[[1]]) || !nzchar(u$referral_code[[1]]))) {
      ...
      if (nrow(q) > 0) {
        # Si choca, sumamos algo random o intentamos +1 manual
        # (Solo para un fix rápido de colisión)
        ref_code <- paste0(ref_code, "x")
      }
...
db_delete_user <- function(user_id) {
  .with_conn({
    tryCatch({
      # Cast to character to avoid list-column/64bit issues with some drivers
      uid <- as.character(user_id)
      
      DBI::dbExecute(con, "delete from sessions where user_id=$1", params = list(uid))
      DBI::dbExecute(con, "delete from password_resets where user_id=$1", params = list(uid))
      DBI::dbExecute(con, "delete from users where id=$1", params = list(uid))
      
      message("DEBUG: Successfully deleted user ID: ", uid)
      return(TRUE)
    }, error = function(e) {
      message("Fatal Error in db_delete_user: ", e$message)
      return(FALSE)
    })
  })
}
